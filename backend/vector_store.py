...
class VectorStore:
    ...
    def search(self, 
        ...
        # Clamp n_results to actual collection size — ChromaDB raises if you
        # request more results than documents exist in the collection.
        try:
            doc_count = self.course_content.count()
            if doc_count == 0:
                return SearchResults(documents=[], metadata=[], distances=[])
            search_limit = min(search_limit, doc_count)
        except Exception:
            pass

        try:
            # Never pass where=None — ChromaDB 1.x raises on explicit None.
            # Omit the parameter entirely when there is no filter.
            query_kwargs: dict = {
                "query_texts": [query],
    ...
    def get_course_outline(self, course_name: str) -> Optional[Dict[str, Any]]:
        """Get complete course outline: title, course link, and full lesson list.

        Args:
            course_name: Course name (partial matches work via semantic search)

        Returns:
            Dict with 'title', 'course_link', and 'lessons' list, or None if not found.
            Each lesson has 'lesson_number', 'lesson_title', and 'lesson_link'.
        """
        import json
        course_title = self._resolve_course_name(course_name)
        if not course_title:
