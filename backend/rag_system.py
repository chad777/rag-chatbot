...
class RAGSystem:
    ...
    def query(self, query: str, session_id: Optional[str] = None,
              image_data: Optional[str] = None, image_media_type: Optional[str] = None) -> Tuple[str, List]:
        """
        ...
        Returns:
            Tuple of (response, sources list with dict objects containing text and url)
        """
        ...
        # Debug: Log sources type and content
        print(f"DEBUG rag_system.py - Sources retrieved: {sources}")
        print(f"DEBUG rag_system.py - Sources type: {type(sources)}")
        if sources and len(sources) > 0:
            print(f"DEBUG rag_system.py - First source: {sources[0]}")
            print(f"DEBUG rag_system.py - First source type: {type(sources[0])}")
